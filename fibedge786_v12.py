...
MANUAL_SCAN_URL='https://github.com/adesh-dhandre/fibedge786/actions/workflows/fibedge_refresh_quality.yml'
...
def home():
 ...
 return render_template_string(HTML,filters=FILTERS,manual_scan_url=MANUAL_SCAN_URL,sel=sel,total=len(df),open_n=len(od),near_n=len(nd),wait_n=len(wd),strong=qrows(strong),good=qrows(good),good_n=len(good),qwatch=len(qw),open_rows=rows(od),near_rows=rows(nd),watch_rows=rows(ww),search_rows=rows(sd),q=q,ql=ql,ol=ol,wl=wl,latest=latest,cards=cards,table=table)
