...
# Re-test the strongest V6 confirmation families at a 1.50 target.
FILTERS = {
...
def simulate_from_next_session(row, df):
    ...
    for j in range(start_idx, len(df)):
        ...
        if (date - entry_date).days > MAX_HOLD_CALENDAR_DAYS:
            return {
                "Outcome150": "TIMEOUT",
                "Entry150Date": entry_date,
                "Exit150Date": date,
                "Hold150Days": (date - entry_date).days,
            }
...
def summarize(df):
    r = df[df["Outcome150"].isin(["WIN", "LOSS"])]
    n = len(r)
    wins = int((r["Outcome150"] == "WIN").sum())
    losses = int((r["Outcome150"] == "LOSS").sum())

    return {
        "Resolved": n,
        "Wins": wins,
        "Losses": losses,
        "Timeouts": int((df["Outcome150"] == "TIMEOUT").sum()),
...
def yearly(df):
    x = df[df["Outcome150"].isin(["WIN", "LOSS"])].copy()
...
def main():
    ...
    for batch_no, start in enumerate(range(0, len(symbols), BATCH_SIZE), 1):
        ...
        print(f"Target1.50 batch {batch_no}/{total_batches} ({len(batch)} symbols)")
    ...
    out.to_csv(RESULTS / "v7_target150_pattern_summary.csv", index=False)

    print("\n=== V7 TARGET 1.50 SUMMARY ===")
