...
const ALLOWED = new Set(["ALL","CLASSIC","CLEAN","PREMIUM","PREMIUMPLUS","STRATEGY05"]);
...
exports.handler = async (event) => {
  ...
  if (!ALLOWED.has(strategy)) return json(400, {ok:false,error:"Unknown strategy"});

  const token = process.env.GITHUB_ACTIONS_TOKEN;
  if (!token) {
    return json(503, {ok:false,error:"GitHub Actions trigger is not configured on the server."});
  }

  const requestId = crypto.randomBytes(12).toString("hex");
  const response = await fetch(
    "https://api.github.com/repos/adesh-dhandre/fibedge786/actions/workflows/fibedge_website_scan.yml/dispatches",
    {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ref: "master",
        inputs: { strategy, request_id: requestId }
      })
    }
  );
